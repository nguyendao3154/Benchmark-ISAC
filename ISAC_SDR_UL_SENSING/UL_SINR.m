%% SMALL V version
function ul_SINR = UL_SINR(BETA, STEERING, vs, G, p, K) 
    NumTgts = size(vs, 2);
    ...
    E = Interference_Plus_Noise_UL(BETA, STEERING, VS, G, p, K);

    ul_SINR =  abs(p(K)*G(:,K)'*inv(E)*G(:,K));
end

%% BIG V version
% function radar_SINR = Radar_SINR(BETA, STEERING, VC, VS, M) 
%     NumElemt = size(VC,1);
%     NumTgts = size(VS, 3);
%     NumDL = size(VC, 3);
% 
%     D = Interference_Plus_Noise_Radar(BETA, STEERING, VC, VS, M);
% 
